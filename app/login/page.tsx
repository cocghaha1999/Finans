...
function LoginContent() {
  console.log('LoginContent component loading...')
  ...
  const isOnline = true
  const saveCredentialsForOffline = (user: any, email: string, password: string) => {}
  const loginOffline = (email: string, password: string, rememberMe: boolean) => false
    ...
    ;(async () => {
      ...
      try {
        ...
        if (isLogin) {
          ...
          // Yeni kullanıcı için Firestore'da user document oluştur
          if (db && cred.user) {
            try {
              ...
              console.log('User document created successfully for:', cred.user.uid)
            } catch (error) {
  ...
  toast({ variant: "destructive", title: "Hata", description: "Kimlik doğrulama yapılandırılmadı" })
    ...
    try {
      ...
      // OAuth ile giriş yapan kullanıcı için user document oluştur/güncelle
      if (db && result.user) {
        try {
          ...
          await setDoc(userDocRef, {
            ...
            provider: provider
          }, { merge: true }) // merge: true ile mevcut data'yı korur
          console.log('OAuth user document created/updated for:', result.user.uid)
        } catch (error) {
          console.error('Error creating/updating OAuth user document:', error)
        }
      }
