...
export default function AppHeader() {
  ...
  unsubCards = watchCards(user.uid, (list: BankCard[]) => {
        ...
        try {
          ...
          setTransactions(localTxns ? JSON.parse(localTxns) : [])
  ...
  const end = d >= day ? new Date(y, m + 1, day - 1) : new Date(y, m, day - 1)
        ...
        const periodTotal = txns.filter((t) => { const td = new Date(t.date); return td >= start && td <= end })
          .reduce((acc, t) => acc + (t.type === "gelir" ? t.amount : -t.amount), 0)
        setTotalBalance(periodTotal)
      } catch {}
    }
  ...
  // Recompute period balance whenever transactions or resetDay change
  useEffect(() => {
  ...
  const end = d >= day ? new Date(y, m + 1, day - 1) : new Date(y, m, day - 1)
    ...
    const current = transactions.filter((t) => { const td = new Date(t.date); return td >= start && td <= end })
      .reduce((acc, t) => acc + (t.type === "gelir" ? t.amount : -t.amount), 0)
  ...
  prevEnd.setHours(23, 59, 59, 999)
    ...
    // Apply carryover first so reconcile keeps final balance == desired
    const carryoverEffect = prevTotal // positive increases balance, negative decreases
    const delta = val - (current + carryoverEffect)

    const carryTxn = Math.abs(prevTotal) > 0 ? {
      id: crypto.randomUUID(),
      type: prevTotal > 0 ? "gelir" : "gider",
      amount: Math.abs(prevTotal),
      description: prevTotal > 0 ? "Geçen Dönem Devri (Gelir)" : "Geçen Dönem Devri (Gider)",
      category: "Devreden",
      // Put carryover on the first day of this period for clarity
      date: new Date(start.getTime()).toISOString().slice(0,10),
      createdAt: Date.now(),
    } as any : null

    const reconTxn = Math.abs(delta) > 0 ? {
      id: crypto.randomUUID(),
    ...
    if (u?.uid && isFirestoreReady()) {
      if (carryTxn) {
        await addTransaction(u.uid, carryTxn)
        await addNotification(u.uid, { message: `${carryTxn.description} • ₺${carryTxn.amount.toLocaleString("tr-TR")}` , type: "info", read: false, timestamp: Date.now() })
      }
      if (reconTxn) {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <div className="hidden md:flex items-center space-x-2 px-2 py-2 bg-primary/10 rounded-lg border border-primary/20">
                ...
                <Popover open={reconcileOpen} onOpenChange={setReconcileOpen}>
                  <PopoverTrigger asChild>
                    <Button size="sm" variant="outline" className="h-7 px-2 text-xs ml-2">Eşitle</Button>
                  </PopoverTrigger>
              ...
              <Button variant="ghost" size="icon" onClick={openGlobalCalendar}>
